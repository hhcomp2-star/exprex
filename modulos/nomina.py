...
def confirmar_actualizacion_modal():
    ...
    * **Teléfono:** {datos['telefono_actual']} ➡️ **{datos['telefono_nuevo']}** * **Correo:** {datos['correo_actual']} ➡️ **{datos['correo_nuevo']}**
    * **Dirección:** {datos['direccion_actual']} ➡️ **{datos['direccion_nuevo']}**
    * **Banco:** {datos['banco_actual']} ➡️ **{datos['banco_nuevo']}** * **Cuenta:** {datos['num_cuenta_actual']} ➡️ **{datos['num_cuenta_nuevo']}**
    """)
    
    # Solo mostramos datos vehiculares si el nuevo cargo es Conductor
    ...
    with col2:
        if st.button("✅ Confirmar y Guardar", type="primary", use_container_width=True):
            try:
                ...
                # UPDATE 1: Tabla Usuarios (Campos maestros generales + Departamento y Rol)
                sql_usuarios = "UPDATE usuarios SET departamento = ?, rol = ?, telefono = ?, email = ?, direccion = ?, banco = ?, numero_cuenta = ? WHERE cedula = ?"
                cursor.execute(sql_usuarios, (
                    datos['departamento_nuevo'], datos['cargo_nuevo'], 
                    datos['telefono_nuevo'], datos['correo_nuevo'], datos['direccion_nuevo'], datos['banco_nuevo'], datos['num_cuenta_nuevo'], datos['cedula']
                ))
                
                # UPDATE 2: Tabla Conductores (Manejo condicional)
                if datos['cargo_nuevo'] == 'Conductor':
                    # Verificamos si ya existía en la tabla conductores por si cambió de cargo administrativo a chofer
                    cursor.execute("SELECT 1 FROM conductores WHERE cedula = ?", (datos['cedula'],))
                    existe = cursor.fetchone()
                    
                    if existe:
                        sql_conductores = "UPDATE conductores SET vehiculo = ?, placa = ?, propio = ?, vence_certificado = ?, vence_rotc = ?, capacidad_carga = ? WHERE cedula = ?"
                        ...
                        sql_insert_cond = "INSERT INTO conductores (cedula, vehiculo, placa, propio, disponible, vence_certificado, vence_rotc, capacidad_carga) VALUES (?,?,?,?,'Sí',?,?,?)"
                    ...
                    # Si ya no es Conductor, lo removemos de la tabla operativa de conductores para liberar placa
                    cursor.execute("DELETE FROM conductores WHERE cedula = ?", (datos['cedula'],))
...
# ====================================================
def mostrar_modulo_nomina(): # Mantenemos el nombre de la llamada original para no romper tu main.py

    # Listas maestros del negocio
    ...
    with tab_activos:
        conexion = sqlite3.connect(personal_base_datos)
        # Traemos toda la nómina activa base
        query = """
        ...
        if df_activos.empty:
            ...
            # 🛠️ FILTROS CRUZADOS EN PARALELO (Ocupan una sola fila para no quitar espacio)
            ...
            # El buscador individual ahora es ultra inteligente: obedece a ambos filtros simultáneamente
            ...
            if seleccion != "-- Seleccione para ver Ficha Detallada --":
                ...
                with col_personal:
                    ...
                    st.markdown(f"* **Dirección de Habitación:**")
                    st.info(persona['Dirección'] if persona['Dirección'] else "No registrada.")
                    st.markdown("#### Datos Financieros")
                    st.markdown(f"* **Banco:** {persona['Banco']}") if persona['Banco'] is not None else st.markdown(f"* **Banco:** Por recibir información")
                    st.markdown(f"* **Cuenta:** {persona['Cuenta']}") if persona['Cuenta'] is not None else st.markdown(f"* **Cuenta:** Por recibir información")
    ...
    with tab_crear:
        ...
        with st.form("form_registro_conductor", clear_on_submit=True):
            ...
            with col_u2:
                # El 'rol' ahora se guarda con el nombre del cargo
            ...
            # --- Bloque Condicional Informativo ---
            st.markdown("---")
            st.markdown("##### 🚛 Datos de Vehículo (Exclusivo para Choferes)")
            st.caption("Nota: Si el cargo seleccionado arriba no es 'Chofer', el sistema ignorará estos campos automáticamente.")
            ...
            if st.form_submit_button("Guardar Registro"):
                if not c_cedula or not c_nombre or not c_clave:
                    ...
                    try:
                        ...
                        # INSERT 1: Siempre se registra en usuarios (Guardando departamento y cargo)
                        cursor.execute("""
                            INSERT INTO usuarios (cedula, nombre, departamento, rol, telefono, email, direccion, banco, num_cuenta, contrasena, activo) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Sí')
                        """, (c_cedula, c_nombre, c_departamento, c_cargo, c_telefono, c_email, c_direccion, c_banco, c_num_cuenta, c_clave))
                        
                        # INSERT 2: Solo va a la tabla de conductores si el cargo es Chofer
                        if c_cargo == "Chofer":
                            cursor.execute("""
    ...
    # ====================================================
    # PESTAÑA 3: MODIFICAR DATOS (SANEADA Y ALINEADA)
    # ====================================================
    with tab_modificar:
        ...
        if df_activos.empty:
            ...
            if cedula_mod != "-- Seleccione --":
                ...
                # Gestión corta de disponibilidad operativa solo si es Chofer
                if cargo_act == "Chofer":
                    ...
                    if st.button("Actualizar Solo Disponibilidad", use_container_width=True):
                        conexion = sqlite3.connect(personal_base_datos)
                        cursor = conexion.cursor()
                        cursor.execute("UPDATE conductores SET disponible = ? WHERE cedula = ?", (nuevo_estado, cedula_mod))
                        conexion.commit()
                        conexion.close()
                ...
                with st.form(key="form_modificar", clear_on_submit=False):
                    ...
                    nuevo_telefono = st.text_input("Teléfono:", value=tel)
                    nuevo_correo = st.text_input("Correo Electrónico:", value=corr)
                    nuevo_direccion = st.text_area("Dirección:", value=dire)
                    ...
                    # Campos vehiculares mutables o visibles condicionalmente si el nuevo cargo es Chofer
                    ...
                    nuevo_certificado = st.text_input("Vencimiento Certificado Médico (AAAA-MM-DD):", value=cert if cert else "2026-01-01")
                    nuevo_rotc = st.text_input("Vencimiento ROTC (AAAA-MM-DD):", value=rotc if rotc else "2026-01-01")
        ...
        if df_activos.empty:
            ...
            if cedula_baja != "-- Seleccione una Cédula --":
                ...
                if st.button("🔥 Aplicar Baja Definitiva", type="primary", disabled=not check_legal):
                    ...
                    conexion = sqlite3.connect(personal_base_datos)
                    cursor = conexion.cursor()
                    # Actualiza la desincorporación
                    cursor.execute("UPDATE usuarios SET activo = 'No', fecha_baja = ? WHERE cedula = ?", (fecha_hoy, cedula_baja))
                    conexion.commit()
                    conexion.close()
