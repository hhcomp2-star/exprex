...
def mostrar_modulo_reporte_general():
    ...
    # =========================================================================
    # 1. CONEXIÓN A BASE DE DATOS Y EXTRACCIÓN DE DATOS REALES (POSTGRESQL)
    # =========================================================================
    
    # Inicializamos DataFrames vacíos con sus columnas para evitar errores de referencia
    ...
    try:
        with obtener_conexion_db() as conexion:
            # A. Leer Viajes (Ingresos de fletes Descuentos y Pagos a los Choferes)
            with conexion.cursor() as cursor:
                # SQL compatible con Postgres usando %s
                query_viajes = "SELECT fecha_despacho, monto_flete_usd, descuento_usd, pago_chofer_usd FROM viajes"
                cursor.execute(query_viajes)
                filas = cursor.fetchall()
                if cursor.description:
                    columnas = [desc[0] for desc in cursor.description]
                    df_viajes_empresa = pd.DataFrame(filas, columns=columnas)
                    df_viajes_empresa['fecha_dt'] = pd.to_datetime(df_viajes_empresa['fecha_despacho'])

            # B. Leer Egresos de Combustible
            with conexion.cursor() as cursor:
                query_combustible = "SELECT fecha, costo_usd FROM control_combustible"
                cursor.execute(query_combustible)
                filas = cursor.fetchall()
                if cursor.description:
                    columnas = [desc[0] for desc in cursor.description]
                    df_combustible = pd.DataFrame(filas, columns=columnas)
                    df_combustible['fecha_dt'] = pd.to_datetime(df_combustible['fecha'])

            # C. Leer Egresos Operativos de Viaje
            with conexion.cursor() as cursor:
                query_gastos_op = "SELECT fecha, tipo_gasto, monto_usd FROM gastos_operativos_viaje"
                cursor.execute(query_gastos_op)
                filas = cursor.fetchall()
                if cursor.description:
                    columnas = [desc[0] for desc in cursor.description]
                    df_gastos_op = pd.DataFrame(filas, columns=columnas)
                    df_gastos_op['fecha_dt'] = pd.to_datetime(df_gastos_op['fecha'])

            # D. Leer Gastos Generales
            with conexion.cursor() as cursor:
                query_gastos_gen = "SELECT fecha, categoria, monto_usd FROM gastos"
                cursor.execute(query_gastos_gen)
                filas = cursor.fetchall()
                if cursor.description:
                    columnas = [desc[0] for desc in cursor.description]
                    df_gastos_gen = pd.DataFrame(filas, columns=columnas)
    ...
    def mapear_moneda(df):
        # Usamos applymap (o map en versiones muy nuevas) de forma segura para evitar advertencias de tipado
    ...
    with col_der:
        ...
        st.download_button(
            ...
            data=buffer, 
            file_name="flujo_caja_exprex_2026.xlsx", 
    ...
    if fecha_inicio <= fecha_fin:
        ...
        ingresos_rango = df_viajes_empresa[
            (df_viajes_empresa['fecha_dt'] >= t_inicio) & (df_viajes_empresa['fecha_dt'] <= t_fin)
        ]['monto_flete_usd'].sum()
       
        egreso_descuento_rango = df_viajes_empresa[
            (df_viajes_empresa['fecha_dt'] >= t_inicio) & (df_viajes_empresa['fecha_dt'] <= t_fin)
        ]['descuento_usd'].sum()
        
        egreso_chofer_rango = df_viajes_empresa[
            (df_viajes_empresa['fecha_dt'] >= t_inicio) & (df_viajes_empresa['fecha_dt'] <= t_fin)
        ]['pago_chofer_usd'].sum()
        
        egreso_comb_rango = df_combustible[
            (df_combustible['fecha_dt'] >= t_inicio) & (df_combustible['fecha_dt'] <= t_fin)
        ]['costo_usd'].sum()
        
        egreso_op_rango = df_gastos_op[
            (df_gastos_op['fecha_dt'] >= t_inicio) & (df_gastos_op['fecha_dt'] <= t_fin)
        ]['monto_usd'].sum()
        
        egreso_gen_rango = df_gastos_gen[
            (df_gastos_gen['fecha_dt'] >= t_inicio) & (df_gastos_gen['fecha_dt'] <= t_fin)
        ]['monto_usd'].sum()
        ...
        # Renderizado del cuadro de control final de forma segura
        def mapear_moneda_rango(val):
            if isinstance(val, (int, float)):
                return f"$ {val:,.2f}"
            return val
            
        st.dataframe(df_rango_resumen.applymap(mapear_moneda_rango), use_container_width=True)
    else:
