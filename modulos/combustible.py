...
def mostrar_modulo_combustible():
    # 🔑 SOLUCIÓN PYLANCE: Inicializamos los DataFrames vacíos al inicio para asegurar que existan siempre
    ...
    try:
        ...
        st.error(f"❌ Error al cargar datos de vehículos: {e}")

    # Creamos la lista para el menú desplegable del registro
    if df_placas is not None and not df_placas.empty:
        opciones_placas = [
            f"{row['placa']} (Asignado a: {row['nombre']})"
            for index, row in df_placas.iterrows()
        ]
        # Listas limpias y únicas para los filtros de búsqueda
        lista_placas_filtro = ["Todos"] + sorted(
            df_placas["placa"].unique().tolist()
        )
        lista_choferes_filtro = ["Todos"] + sorted(
            df_placas["nombre"].unique().tolist()
        )
    else:
    ...
    tab_registrar, tab_historial = st.tabs(
        ["📝 Registrar Carga", "📊 Historial de Consumo"]
    )
    ...
    with tab_registrar:
        with st.form("form_combustible", clear_on_submit=True):
            ...
            with col1:
                vehiculo_selected = st.selectbox(
                    "🚗 Seleccione el Vehículo / Placa:", opciones_placas
                )
                c_fecha = st.date_input("Fecha de la carga", datetime.now())
                c_km = st.number_input(
                    "Kilometraje Actual (Odómetro)", min_value=0, step=1
                )
                c_litros = st.number_input(
                    "Litros Comprados", min_value=0.0, step=0.1
                )

            with col2:
                monto_bs = st.number_input(
                    "Monto Total en Bolívares (Bs.)", min_value=0.0, step=10.0
                )
                ...
                st.number_input(
                    "Tasa BCV Aplicada (Fija)", value=c_tasa, disabled=True
                )
                c_estacion = st.text_input(
                    "Estación de Servicio (Nombre/Ubicación)"
                )

                _, sub_col_centro, _ = st.columns([1, 2, 1])
                with sub_col_centro:
                    boton_gasolina = st.form_submit_button(
                        "Clic aquí para Registrar Combustible",
                        use_container_width=True,
                    )

            st.info(
                f"📊 **Impacto en Costos:** Esta carga equivale a **{c_costo_usd:,.2f} USD** (Calculado automáticamente a {c_tasa:.2f} Bs.)"
            )
            st.caption(
                "💡 **Nota del Sistema:** Para cerrar esta ficha y limpiar la pantalla, simplemente seleccione otra opción en el menú de la izquierda o haga clic en **Inicio**."
            )

            if boton_gasolina:
                if (
                    df_placas.empty
                    or opciones_placas[0] == "No hay vehículos registrados"
                ):
                    st.error(
                        "❌ No se puede registrar combustible si no hay vehículos activos en el sistema."
                    )
                elif c_km == 0 or c_litros == 0.0 or monto_bs == 0.0:
                    st.error(
                        "❌ Todos los campos numéricos (Km, Litros y Monto Bs.) deben ser mayores a cero."
                    )
                else:
                    placa_limpia = vehiculo_selected.split(" ")[0]

                    try:
                        # Abrimos conexión automática con PostgreSQL
                        with obtener_conexion_db() as conexion:
                            with conexion.cursor() as cursor:
                                cursor.execute(
                                    "SELECT cedula FROM conductores WHERE placa = %s",
                                    (placa_limpia,),
                                )
                                ...
                                # Inserción de datos usando marcadores %s
                                cursor.execute(
                                    """
                        ...
                        st.success(
                            f"⛽ Carga guardada con éxito para la unidad **{placa_limpia}**. Total: {round(monto_bs, 2)} Bs."
                        )
                        time.sleep(2)
                        st.rerun()
                    except Exception as e:
    ...
    with tab_historial:
        ...
        # 🔑 SOLUCIÓN PYLANCE: Inicializamos el DataFrame del historial vacío
        df_historial = pd.DataFrame()

        # 📥 Extracción inicial de todo el universo de datos desde PostgreSQL
        try:
            with obtener_conexion_db() as conexion:
                # Modificado: En Postgres, los alias con espacios van entre comillas dobles, no simples.
                query_historial = """
        ...
        if df_historial.empty:
            ...
            # =================================================================
            with st.expander(
                "🔍 Hacer Auditoría Específica (Filtrar por Período, Placa o Chofer)"
            ):
                ...
                with col_f1:
                    ...
                    filtro_placa = st.selectbox(
                        "🚗 Filtrar por Unidad / Placa:",
                        lista_placas_filtro,
                        key="f_comb_placa",
                    )
                with col_f2:
                    filtro_fin = st.date_input(
                        "🗓️ Hasta:", datetime.now().date(), key="f_comb_fin"
                    )
                    filtro_chofer = st.selectbox(
                        "🧑‍✈️ Filtrar por Conductor:",
                        lista_choferes_filtro,
                        key="f_comb_chofer",
                    )

            # --- Proceso de filtrado dinámico en memoria con Pandas ---
            # 1. Filtro por Rango de Fechas
            df_historial["Fecha_dt"] = pd.to_datetime(
                df_historial["Fecha"]
            ).dt.date
            ...
            if filtro_chofer != "Todos":
                df_filtrado = df_filtrado[
                    df_filtrado["Reportado Por"] == filtro_chofer
                ]
            ...
            with m_litros:
                st.metric(
                    label="⛽ Volumen Surtido (Período)",
                    value=f"{total_litros:,.1f} Ltrs",
                )
            with m_usd:
                st.metric(
                    label="💵 Gasto en Combustible (USD)",
                    value=f"$ {total_usd:,.2f}",
                )
            with m_bs:
                st.metric(
                    label="🇻🇪 Gasto en Combustible (Bs)",
                    value=f"{total_bs:,.2f} Bs",
                )
            ...
            if df_filtrado.empty:
                st.warning(
                    "⚠️ No se encontraron cargas de combustible que coincidan con los filtros seleccionados."
                )
            else:
                st.dataframe(
